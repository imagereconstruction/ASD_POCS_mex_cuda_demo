function image = Forbild_2D_256()
% phantom image size setting
N = 256;
NC = (N-1)/2;
ratio = 0.1; 
% phantom value
oR = 1; % add left ear 
oL = 1; % add right ear
phantom =  analytical_phantom(oR, oL);

% phantom coord
x = ((0:N-1)-NC)*ratio;
y = ((0:N-1)-NC)*ratio;
xcoord = ones(N,1)*x;
ycoord = transpose(y)*ones(1,N);
%
image = discrete_phantom(xcoord,ycoord,phantom);
image = flipud(image);
