	...
	const mxArray *prhs[])
{
	...
	//else
	//{
	//	mexPrintf("GPU failed!\n");
	//	//printf("GPU failed!\n");
	//}
	if (nrhs>2 || nrhs <=1)
	{
		mexPrintf("Usage: [x,sc] = Atx_cone_mf_gpu(Y,parameters)\n");
		mexPrintf("Y: a frame of projection of size na*nb\n");
		...
		mexPrintf("output  x: back-projection data (size nx*ny*nz)\n");
		mexPrintf("output sc: col sum of system matrix\n");
		mexErrMsgTxt("Error usage!");
		return;
	}
	
	int nx,ny,nz,na,nb;
    float *X,*y,*sc,cos_phi,sin_phi,SO,OD,scale,det,dz,phi;
	...
	if((numelOfInputPrj) !=(na*nb))
	{
		mexPrintf("Size of the first input data (Y) mismatch na*nb! Please check!\n");
		mexErrMsgTxt("Error size of input argument Y!");
		return;
	}
    /************************************************************************/
	cos_phi = cos(phi);
	...
	plhs[0]=mxCreateNumericMatrix(nx*ny*nz,1,mxSINGLE_CLASS,mxREAL);
    X=(float*)mxGetData(plhs[0]);

	plhs[1]=mxCreateNumericMatrix(nx*ny*nz,1,mxSINGLE_CLASS,mxREAL);
    sc = (float*)mxGetData(plhs[1]);
	...
	//mexPrintf("OD=%f,SO=%f,nx=%d,ny=%d,nz=%d,vxlsize=%f,phi=%f,det=%f,na=%d,nb=%d\n",OD,SO,nx,ny,nz,scale,phi,det,na,nb);
	
	Atx_cone_mf_gpu_new(X,y,sc,cos_phi,sin_phi,y_det,z_det,SO,OD,scale,det,det,dz,nx,ny,nz,na,nb);
